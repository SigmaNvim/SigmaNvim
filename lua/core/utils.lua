...
M.load_config = function()
  ...
  local sigmarc_exists, sigmarc = pcall(require, "custom.sigmarc")

  if sigmarc_exists then
    -- merge user config if it exists and is a table; otherwise display an error
    if type(sigmarc) == "table" then
      config.mappings = M.remove_disabled_keys(sigmarc.mappings, config.mappings)
      config = merge_tb("force", config, sigmarc) or {}
    else
      error "sigmarc must return a table!"
    end
  end
...
M.remove_disabled_keys = function(sigmarc_mappings, default_mappings)
  if not sigmarc_mappings then
    return default_mappings
  end

  -- store keys in a array with true value to compare
  local keys_to_disable = {}
  for _, mappings in pairs(sigmarc_mappings) do
...
M.packer_sync = function(...)
  local git_exists, git = pcall(require, "sigma.utils.git")
  local defaults_exists, defaults = pcall(require, "sigma.utils.config")
  ...
  if git_exists and defaults_exists then
    ...
    if current_branch_name:match(defaults.snaps.base_snap_branch_name .. "(.+)" .. "$") then
      vim.api.nvim_echo({
        ...
        { "PackerSync" },
        {
          " on a SigmaSnapshot. This will cause issues if SigmaNvim dependencies contain "
  ...
  if packer_exists then
    ...
    if old_style_options then
      vim.notify_once("SigmaNvim: This plugin syntax is deprecated, use new style config.", "Error")
    end
  else
